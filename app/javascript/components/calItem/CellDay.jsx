...
class CellDay extends React.Component{
  ...
  render(){
    const monthM = ( "0" + this.state.month ).slice(-2)
    ...
    const prevMonthEndDate = new Date(this.state.year, this.state.month-1, 0 );
    ...
    for ( let d=0 ; d<42 ; d++ ){
          if ( d<startDay ) {
            ...
            let ymd = this.state.year + "-" + prevMonth + "-" + prevDay
            cellDay.push(<div 
            ...
            let nextMonth = ("0" + (this.state.month + 1)).slice(-2)
            let ymd = this.state.year + "-" + nextMonth + "-" + nextDayN
            cellDay.push(<div 
              className="cal-cell disabled" 
              id={ymd} 
              key={d}
            ...
            let ymd = this.state.year + "-" + monthM + "-" + dayThisMonth
            cellDay.push(<div 
              className="cal-cell" 
              id={ymd} 
              key={d}
